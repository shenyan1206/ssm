...
//constructor
function M2(){
	console.log("M2 command started");
};

//class functions
M2.prototype.processCommand = function(req) 
{
...
M2.prototype._parseRequest = function(req)
{
	...
	if(this.input_format_flag == "0") //binary
		this.encrypted_message = BufferHelper.extract(req, this.message_length);
	else if(this.input_format_flag == "1") //hex string
		this.encrypted_message = Buffer.from(BufferHelper.extract(req, this.message_length).toString(), "hex"); //read N hex covert to Buffer
...
M2.prototype._process = function()
{
	...
	output_iv = "";
	if(this.mode == "00"){ //ECB
		this.decrypted_message = CryptoHelper.tdes_dec_ecb(dukpt_key, this.encrypted_message);
	}
	else{ //CBC
		this.decrypted_message = CryptoHelper.tdes_dec_cbc(dukpt_key, this.encrypted_message, this.iv);
		output_iv = Buffer.alloc(8).toString("hex"); //all zero
	}
	
	this.decrypted_message_length = Buffer.alloc(2);
	this.decrypted_message_length.writeUInt16BE(this.decrypted_message.length); //4H, or 2 Byte, buffer

	this.output = output_iv + //output iv, all 0
					this.decrypted_message_length.toString("hex") +
					this.decrypted_message.toString("hex");
	
	this.output = this.output.toUpperCase();
}


M2.prototype._formatOutput = function()
{
